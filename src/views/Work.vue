    ...
    <div>
        <div class="surface-section px-4 py-8 md:px-6 lg:px-8">
            ...
            <div class="text-700 text-center">
                ...
                <div class="text-700 text-2xl mb-5" v-if="!work">Describe con buen detalle el trabajo que realizaras y la IA completara el paso de las tareas</div>
                ...
                <DataTable :value="tasks" editMode="cell" @cell-edit-complete="onCellEditComplete" rowGroupMode="rowspan" groupRowsBy="description" sortMode="single" sortField="id" :sortOrder="1" tableStyle="min-width: 50rem">
                    ...
                    <Column field="routine" header="Rutinario" style="min-width: 50px"></Column>
...
const createIperc = async (task) => {
    console.log(task)
    const response = await fetch(`${import.meta.env.VITE_URL}/${import.meta.env.VITE_VERSION}/iperc`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'x-access-token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1NzRmMDQ5ZWJiZDFmNmUwMmM0MTlhOCIsImlhdCI6MTcwMjE2OTIwOCwiZXhwIjoxNzMzMjczMjA4fQ.P4kv73SgBrqMgZYwZT-XzjSpSyWVu0dnbHpQN4CycMw'
...
const getWorkById = async (workId) => {
    const response = await fetch(`${import.meta.env.VITE_URL}/${import.meta.env.VITE_VERSION}/work/${workId}`, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
            'x-access-token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1NzRmMDQ5ZWJiZDFmNmUwMmM0MTlhOCIsImlhdCI6MTcwMjE2OTIwOCwiZXhwIjoxNzMzMjczMjA4fQ.P4kv73SgBrqMgZYwZT-XzjSpSyWVu0dnbHpQN4CycMw'
    ...
    const data = await response.json()
    const newTasks = data.work.tasks
...
const deleteTask = async (task) => {
    const response = await fetch(`${import.meta.env.VITE_URL}/${import.meta.env.VITE_VERSION}/task/${task._id}`, {
        method: 'DELETE',
        headers: {
            'Content-Type': 'application/json',
            'x-access-token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1NzRmMDQ5ZWJiZDFmNmUwMmM0MTlhOCIsImlhdCI6MTcwMjE2OTIwOCwiZXhwIjoxNzMzMjczMjA4fQ.P4kv73SgBrqMgZYwZT-XzjSpSyWVu0dnbHpQN4CycMw'
...
const createTask = async (task, workId) => {
    const response = await fetch(`${import.meta.env.VITE_URL}/${import.meta.env.VITE_VERSION}/task`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'x-access-token': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY1NzRmMDQ5ZWJiZDFmNmUwMmM0MTlhOCIsImlhdCI6MTcwMjE2OTIwOCwiZXhwIjoxNzMzMjczMjA4fQ.P4kv73SgBrqMgZYwZT-XzjSpSyWVu0dnbHpQN4CycMw'
